...
async def stream_with_callback(
    ...
    **kwargs,
    ...
    """Stream Claude response, calling on_chunk(text) for each token.

    Simpler than stream_with_retry+queue: direct callback, no background task.
    """
    max_retries = settings.anthropic_max_retries
    for attempt in range(max_retries + 1):
        if cancel and cancel.is_set():
        ...
        try:
            ...
            async with client.messages.stream(**kwargs) as stream:
                async for chunk in stream.text_stream:
                    if cancel and cancel.is_set():
                        raise CancelledError("Analysis cancelled during stream")
                    full_text += chunk
                    if on_chunk:
...
async def stream_with_retry(
    ...
    **kwargs,
    ...
    """Stream Claude response with rate limiting + retry.

    Yields tokens to on_token queue as they arrive.
    Returns (full_text, usage) when done.
    """
    max_retries = settings.anthropic_max_retries
    for attempt in range(max_retries + 1):
        if cancel and cancel.is_set():
