"use client";

import Link from "next/link";
import { useAuth } from "@/lib/auth";
import { NavLinks } from "@/components/nav-links";
import { UserMenu } from "@/components/user-menu";
import { AuthPage } from "@/components/auth-page";

export function AppShell({ children }: { children: React.ReactNode }) {
  const { user, loading } = useAuth();

  // Loading state — minimal branded spinner
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center min-h-screen">
        <div className="text-center animate-pulse-slow">
          <div className="w-14 h-14 mx-auto flex items-center justify-center border border-[var(--primary-container)]">
            <span className="font-heading text-[var(--primary)] font-bold text-2xl">L</span>
          </div>
        </div>
      </div>
    );
  }

  // Not authenticated — show full-page login
  if (!user) {
    return (
      <div className="flex-1 flex flex-col min-h-screen">
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8">
          <AuthPage />
        </main>
        <footer className="border-t border-[var(--outline-variant)]/20 bg-[var(--bg)]">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 text-center">
            <span className="text-[10px] tracking-[0.15em] uppercase text-[var(--muted)]">
              LITIGIA — Institutional Grade Legal Tech
            </span>
          </div>
        </footer>
      </div>
    );
  }

  // Authenticated — full app
  return (
    <>
      <header className="sticky top-0 z-50 bg-[var(--bg)]/95 backdrop-blur-md border-b border-[var(--outline-variant)]/30 relative">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 md:py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 md:gap-4 group shrink-0">
            <div className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center border border-[var(--primary-container)] group-hover:border-[var(--primary)] transition-colors">
              <span className="font-heading text-[var(--primary)] font-bold text-base md:text-lg">L</span>
            </div>
            <div className="hidden sm:block">
              <h1 className="text-lg font-bold tracking-[0.2em] text-[var(--on-surface)]">LITIGIA</h1>
              <p className="text-[9px] tracking-[0.3em] uppercase text-[var(--primary-container)] opacity-70">
                Institutional Grade Legal Tech
              </p>
            </div>
          </Link>
          <NavLinks />
          <div className="flex items-center gap-2 md:gap-3 shrink-0">
            <Link href="/settings" className="text-[var(--muted)] hover:text-[var(--primary)] transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
              </svg>
            </Link>
            <UserMenu />
          </div>
        </div>
      </header>

      <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8 py-6 md:py-10">
        {children}
      </main>

      <footer className="border-t border-[var(--outline-variant)]/20 bg-[var(--bg)]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
            <span className="font-heading text-sm italic text-[var(--primary-container)]">Litigia</span>
            <span className="text-[10px] tracking-[0.15em] uppercase text-[var(--muted)] ml-3">
              Institutional Grade Legal Tech
            </span>
          </div>
          <div className="flex gap-4 md:gap-6 text-[11px] tracking-wide uppercase text-[var(--muted)]">
            <span className="hover:text-[var(--primary)] transition-colors cursor-pointer">Privacy</span>
            <span className="hover:text-[var(--primary)] transition-colors cursor-pointer">Terms</span>
            <span className="hover:text-[var(--primary)] transition-colors cursor-pointer">Documentation</span>
          </div>
        </div>
      </footer>
    </>
  );
}
